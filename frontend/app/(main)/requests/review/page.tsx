...
export default function ReviewRequestsPage() {
  ...
  const isHHRMD = user?.role === 'HHRMD';
  const canReview = isHRMO || isHHRMD;
  ...
  const loadRequests = async () => {
    ...
    // Filter out disciplinary requests for HRMO
    let filteredRequests = result.requests;
    if (isHRMO) {
      filteredRequests = filteredRequests.filter((r) =>
        !r.separation // HRMO cannot review disciplinary requests
      );
    }
  ...
  const handleReject = async (id: string, type: string) => {
    ...
    if (!reason) return;
    try {
      await requestService.rejectRequest(id, reason, type);
      loadRequests();
    } catch (error) {
  ...
  const handleReturn = async (id: string, type: string) => {
    const reason = prompt('Enter reason for return:');
    if (!reason) return;
    try {
      await requestService.returnRequest(id, reason, type);
      loadRequests();
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">You do not have permission to review requests</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            {isHRMO ? 'HR Requests for Review' : 'All Requests for Review'}
          </h1>
          <p className="text-gray-600 mt-1">
            {isHRMO
              ? 'Review and approve HR requests (excluding disciplinary matters)'
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <p className="text-gray-500">No requests pending review</p>
          </div>
        ) : (
                ...
                {requests.map((request) => {
                  ...
                  return (
                    <tr key={request.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{typeLabel}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            onClick={() => handleReject(request.id, type)}
                            className="text-red-600 hover:text-red-900 text-sm font-medium"
                          >
                            Reject
                          </button>
                          <button
                            onClick={() => handleReturn(request.id, type)}
