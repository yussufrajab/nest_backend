...
export default function SeparationRequestsPage() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    const result = await requestService.getRequests({ type: 'separation' });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Separation</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">Separation Requests (Termination/Dismissal)</h1>
          ...
          {canCreateRequest && (
            <button
              onClick={() => router.push('/requests/separation/new')}
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <p className="text-gray-500">No separation requests found</p>
          </div>
        ) : (
